...
'''
def makeNewMergedIMG(arrayofImg,name1,img2,name2,threshold,parts): # need to add parts to function call
    ArrayIMG2 = []
    countOfGood =0
    name=name1+name2
    black = Image.open("Black_Image.jpg")
...
def addColumToMatrixToSplit(matrix,addedMerged,arrayofpictures,parts,counter): # not used at the moment, adding another row/colum to a matrix
    ...
    for i in arrayofpictures[:]:
        if i!=addedMerged:
            ...
            img1,score1,name1=makeNewMergedIMG(split1,stringy, i,name1,threshold=0.05)
            img2,score2, name2=makeNewMergedIMG(split2,name,addedMerged,stringy,threshold=0.05)
            if score1>score2:
